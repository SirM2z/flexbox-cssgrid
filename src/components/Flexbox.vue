<template>
  <div class="flexbox">
    <el-row>
      <el-col :span="8">
        <h3>父容器</h3>
        <el-form ref="parrentform" :model="parrentform" label-width="100px" class="parrentform">
          <el-form-item label="height">
            <el-slider v-model="parrentform.height" :format-tooltip="formaParrenttHeight" :step="10"></el-slider>
          </el-form-item>
      ...
      <el-col :span="16">
        <div class="main-content">
          <div class="flex-parrent"
            :style="{
              display: 'flex',
              height: parrentform.height == 0 ? 'auto' : parrentform.height * 5 + 'px',
            ...
            <div v-for="(item, index) in child" :key="item.key" class="flex-child"
              :style="{
                width: child_width + '%',
    ...
    <el-row class="clearfix">
      ...
      <div v-for="(item, index) in child" :key="item.key" class="child-conf">
        ...
        <el-row>
          ...
          <el-col :span="15"><el-slider v-model="item.height" :format-tooltip="formatHeight" :step="10"></el-slider></el-col>
        </el-row>
        <el-row>
    ...
    <div class="des">
      <h3>属性描述</h3>
      <p class="cankao">参考 <a href="http://css.doyoe.com/properties/flex/index.htm" target="_blank">css 手册</a></p>
      <p class="cankao">
        目录&nbsp;
      ...
      <p class="cankao">flex: 0 ( flex-grow ) 1 ( flex-shrink ) auto ( flex-basis ); 默认值-( 属性 )</p>
      <p class="cankao">flex-basis 控制着元素在沿着主轴上 grow 和 shrik 之前最终能够变化多大</p>
      <p class="cankao">flex-grow 决定了他将与兄弟元素成比例的增长多少， flex-shrink 决定了缩小多少</p>
      <div class="item" id="flex-direction">
        <div>
          <span class="label">flex-direction :</span><span>设置或检索伸缩盒对象的子元素在父容器中的位置</span>
      ...
      <div class="item" id="flex-wrap">
        <div>
          <span class="label">flex-wrap :</span><span>设置或检索伸缩盒对象的子元素超出父容器时是否换行</span>
      ...
      <div class="item" id="justify-content">
        <div>
          <span class="label">justify-content :</span><span>设置或检索弹性盒子元素在主轴（横轴）方向上的对齐方式</span>
      ...
      <div class="item" id="align-items">
        <div>
          <span class="label">align-items :</span><span>设置或检索弹性盒子元素在侧轴（纵轴）方向上的对齐方式</span>
        ...
        <el-row style="marginTop:25px;">
          <el-col :span="10" :offset="1">
            <el-card :body-style="{ padding: '0px' }">
              ...
              <p class="img-des">align-items: stretch 时每个 子元素 的 height 必须为 auto 否则 height 属性会覆盖 stretch 的效果</p>
            </el-card>
          </el-col>
          <el-col :span="10" :offset="1">
            <el-card :body-style="{ padding: '0px' }">
              <img src="~@/assets/images/flex9.png" class="image">
              <p class="img-des">如果 div 内没有内容，或者子标签内没有内容，将按照每个 div 的底部对齐</p>
            </el-card>
          </el-col>
        </el-row>
      ...
      <div class="item" id="align-content">
        <div>
          <span class="label">align-content :</span><span>设置或检索弹性盒堆叠伸缩行的对齐方式</span>
          <span class="label sub">默认值 :</span><span>stretch</span>
          <span class="label sub">适用于 :</span><span>多行的弹性盒模型容器</span>
        </div>
        <el-table
          class="table"
        ...
        <el-row style="margin:25px auto 0;width:1055px;">
          <el-card :body-style="{ padding: '0px' }">
            ...
            <p class="img-des">flex-wrap 需设为 wrap, 且数量超出一行, 父容器的高度相对于子容器有多余，才能看到效果</p>
          </el-card>
        </el-row>
      </div>
      <div class="item" id="order">
        <div>
          <span class="label">order :</span><span>设置或检索伸缩盒对象的子元素出現的順序</span>
          <span class="label sub">默认值 :</span><span>0</span>
          <span class="label sub">适用于 :</span><span>flex子项和flex容器中的绝对定位子元素</span>
        </div>
        <el-row style="margin:25px auto 0;width:545px;">
          <el-card :body-style="{ padding: '0px' }">
            <img src="~@/assets/images/flex14.png" class="image">
            <p class="img-des">用整数值来定义排列顺序，数值小的排在前面。可以为负值</p>
          </el-card>
        </el-row>
      </div>
      <div class="item" id="flex-grow">
        <div>
          <span class="label">flex-grow :</span><span>设置或检索弹性盒的扩展比率</span>
          <span class="label sub">默认值 :</span><span>0</span>
      ...
      <div class="item" id="flex-shrink">
        <div>
          <span class="label">flex-shrink :</span><span>设置或检索弹性盒的收缩比率（与flex-grow相反）</span>
      ...
      <div class="item" id="flex-basis">
        <div>
          <span class="label">flex-basis :</span><span>设置或检索弹性盒伸缩基准值</span>
        ...
        <el-row style="margin:25px auto 0;width:1000px;">
          <el-card :body-style="{ padding: '0px' }">
            ...
            <p class="img-des">flex-basis 是通过主轴 (main axis) 来影响元素尺寸的</p>
          </el-card>
        </el-row>
      </div>
      <div class="item" id="align-self">
        <div>
          <span class="label">align-self :</span><span>设置或检索弹性盒子元素自身在侧轴（纵轴）方向上的对齐方式</span>
          <span class="label sub">默认值 :</span><span>auto</span>
          <span class="label sub">适用于 :</span><span>flex子项</span>
        </div>
        <el-table
  ...
  data () {
    return {
      ...
      flexGrowTableData: [
        {
          attributes: 'row',
          des: '主轴与行内轴方向作为默认的书写模式。即横向从左到右排列（左对齐）'
        }, {
      ...
      flexWrapTableData: [
        {
          ...
          attributes: 'wrap',
          des: 'flex容器为多行。该情况下flex子项溢出的部分会被放置到新行，子项内部会发生断行'
        }, {
      ...
      justifyContentTableData: [
        {
          ...
          des: '弹性盒子元素将向行起始位置对齐。该行的第一个子元素的主起始位置的边界将与该行的主起始位置的边界对齐，同时所有后续的伸缩盒项目与其前一个项目对齐'
        }, {
          attributes: 'flex-end',
          des: '弹性盒子元素将向行结束位置对齐。该行的第一个子元素的主结束位置的边界将与该行的主结束位置的边界对齐，同时所有后续的伸缩盒项目与其前一个项目对齐'
        }, {
          attributes: 'center',
          des: '弹性盒子元素将向行中间位置对齐。该行的子元素将相互对齐并在行中居中对齐，同时第一个元素与行的主起始位置的边距等同与最后一个元素与行的主结束位置的边距（如果剩余空间是负数，则保持两端相等长度的溢出）'
        }, {
          attributes: 'space-between',
          des: '弹性盒子元素会平均地分布在行里。如果最左边的剩余空间是负数，或该行只有一个子元素，则该值等效于"flex-start"。在其它情况下，第一个元素的边界与行的主起始位置的边界对齐，同时最后一个元素的边界与行的主结束位置的边距对齐，而剩余的伸缩盒项目则平均分布，并确保两两之间的空白空间相等'
        }, {
          attributes: 'space-around',
          des: '弹性盒子元素会平均地分布在行里，两端保留子元素与子元素之间间距大小的一半。如果最左边的剩余空间是负数，或该行只有一个伸缩盒项目，则该值等效于"center"。在其它情况下，伸缩盒项目则平均分布，并确保两两之间的空白空间相等，同时第一个元素前的空间以及最后一个元素后的空间为其他空白空间的一半'
        }
      ],
      alignItemsTableData: [
        {
          attributes: 'flex-start',
          des: '弹性盒子元素的侧轴（纵轴）起始位置的边界紧靠住该行的侧轴起始边界'
        }, {
          attributes: 'flex-end',
          des: '弹性盒子元素的侧轴（纵轴）起始位置的边界紧靠住该行的侧轴结束边界'
        }, {
          attributes: 'center',
          des: '弹性盒子元素在该行的侧轴（纵轴）上居中放置。（如果该行的尺寸小于弹性盒子元素的尺寸，则会向两个方向溢出相同的长度）'
        }, {
          attributes: 'baseline',
          des: '如弹性盒子元素的行内轴与侧轴为同一条，则该值与"flex-start"等效。其它情况下，该值将参与基线对齐'
        }, {
          attributes: 'stretch',
          des: '如果指定侧轴大小的属性值为"auto"，则其值会使项目的边距盒的尺寸尽可能接近所在行的尺寸，但同时会遵照"min/max-width/height"属性的限制'
        }
      ],
      alignContentTableData: [
        {
          attributes: 'flex-start',
          des: '各行向弹性盒容器的起始位置堆叠。弹性盒容器中第一行的侧轴起始边界紧靠住该弹性盒容器的侧轴起始边界，之后的每一行都紧靠住前面一行'
        }, {
          attributes: 'flex-end',
          des: '各行向弹性盒容器的结束位置堆叠。弹性盒容器中最后一行的侧轴起结束界紧靠住该弹性盒容器的侧轴结束边界，之后的每一行都紧靠住前面一行'
        }, {
          attributes: 'center',
          des: '各行向弹性盒容器的中间位置堆叠。各行两两紧靠住同时在弹性盒容器中居中对齐，保持弹性盒容器的侧轴起始内容边界和第一行之间的距离与该容器的侧轴结束内容边界与第最后一行之间的距离相等。（如果剩下的空间是负数，则各行会向两个方向溢出的相等距离。）'
        }, {
          attributes: 'space-between',
          des: '各行在弹性盒容器中平均分布。如果剩余的空间是负数或弹性盒容器中只有一行，该值等效于"flex-start"。在其它情况下，第一行的侧轴起始边界紧靠住弹性盒容器的侧轴起始内容边界，最后一行的侧轴结束边界紧靠住弹性盒容器的侧轴结束内容边界，剩余的行则按一定方式在弹性盒窗口中排列，以保持两两之间的空间相等'
        }, {
          attributes: 'space-around',
          des: '各行在弹性盒容器中平均分布，两端保留子元素与子元素之间间距大小的一半。如果剩余的空间是负数或弹性盒容器中只有一行，该值等效于"center"。在其它情况下，各行会按一定方式在弹性盒容器中排列，以保持两两之间的空间相等，同时第一行前面及最后一行后面的空间是其他空间的一半'
        }, {
          attributes: 'stretch',
          des: '各行将会伸展以占用剩余的空间。如果剩余的空间是负数，该值等效于"flex-start"。在其它情况下，剩余空间被所有行平分，以扩大它们的侧轴尺寸'
        }
      ],
      alignSelfTableData: [
        {
          attributes: 'auto',
          des: '如果"align-self"的值为"auto"，则其计算值为元素的父元素的"align-items"值，如果其没有父元素，则计算值为"stretch"'
        }, {
          attributes: 'flex-start',
          des: '弹性盒子元素的侧轴（纵轴）起始位置的边界紧靠住该行的侧轴起始边界'
        }, {
